...
public class MaxHeap {
	...
	public void heapify(int pos) {
		if ( isLeaf(pos) )
			return;
		if ( heap[pos].getBalls() < heap[ leftChild(pos) ].getBalls() || heap[pos].getBalls() < heap[ rightChild(pos) ].getBalls() ) {
	...
	public boolean isMaxFromItsChild(int pos) {
		if ( this.heap[pos].getBalls() >= heap[ leftChild(pos) ].getBalls() && this.heap[pos].getBalls() >= heap[ rightChild(pos) ]
						.getBalls() )
			return true;
		return false;
	}
	...
	public void playVirat(int remainingBowls) {
		...
		while (remainingBowls > 0) {
			while (this.isMaxFromItsChild(1) && remainingBowls > 0) {
				...
				this.getMaxBowler().setBalls( this.getMaxBowler().getBalls() - 1 );
				remainingBowls -= 1;
				index += 1;
			}
			this.heapify(1);
